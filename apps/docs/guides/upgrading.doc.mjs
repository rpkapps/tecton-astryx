  ...
  sections: [
    {
      ...
      content: [
        {
          type: 'prose',
          text: 'A Tecton release is one published version of `@tecton/react`: the compiled components, their type declarations, the icon set, the theme and the five stylesheets, built together from one commit. The stylesheet in a release always matches the code in that release — that is why they ship in the same package and are versioned as one thing.',
        },
        {
          type: 'prose',
          text: 'Every release is listed on the [changelog](/changelog).',
        },
      ],
    },
    {
      ...
      content: [
        {
          ...
          rows: [
            ...
            ['A new component, a new prop, a new glyph, a new token', 'minor'],
            [
              'A prop removed or renamed, a default changed, a component removed',
      ...
      content: [
        {
          ...
          items: [
            ...
            'Type-check. Most breaking changes in Tecton are type errors first: a renamed prop, a narrowed union, a required label.',
            "Search your codebase for anything that reaches past the API — a Tecton class name, a redefined token, a style override aimed at a component's internals. Those are the changes that break silently.",
    ...
    },
    {
      ...
      content: [
        ...
        },
        {
          type: 'list',
          items: [
            "**One theme layer on the page.** The host loads exactly one `tokens.css`, and it is the newest version the page carries. Every container then renders in the host's token values, on purpose.",
            "**One component stylesheet per container**, matching the code in that container's bundle. Component styles are content-hashed, so versions coexist without contesting each other.",
            '**One owner of the document root.** The shell calls `configureTectonRoot()`; every container mounts with `scope="nested"`.',
            '**Concurrent versions stay inside one major of the library underneath.** Crossing that line puts two incompatible base layers on the page, and no amount of care in the wrapper fixes it.',
          ],
        },
        {
          type: 'prose',
