 ...
 * names, same props — so the port is **import rewriting**, not translation.
 * Nothing in an example body is reworded, no prop is mapped and no component is
 * renamed. Exactly five things change:
 *
 ...
 *      the fallback are written to the log.
 ...
 *   5. Doc objects are copied verbatim, with `@astryxdesign/core`, `Astryx` and
 *      `astryx` rewritten inside their strings and an `id` (blocks) or `slug`
 *      (pages) added so a page can find the file the doc describes.
 *
 ...
 * there; they are stripped, and every strip is logged.
 */
const GLYPH_UNSUPPORTED = new Set([
  'size',
  'title',
  'titleId',
  'absoluteStrokeWidth',
...
/**
 * The rewrite applied to the *text* of a doc string.
 *
 * Longest first: `@astryxdesign/core` has to become `@tecton/react` before the
 * bare `astryx` rule can turn it into something that is neither.
 */
const rewriteText = text =>
  text
    .replace(/@astryxdesign\/core/g, '@tecton/react')
    .replace(/Astryx/g, 'Tecton')
    .replace(/astryx/g, 'tecton');
  ...
  if (bindings.size > 0) {
    ...
    for (const element of iconElements) {
      for (const attribute of element.attributes.properties) {
        ...
        if (!GLYPH_UNSUPPORTED.has(attribute.name.text)) continue;
        edits.push({
        ...
        record(
          'stripped',
          `${label}: <${element.tagName.getText(file)} ${attribute
            .getText(file)
            .replace(/\s+/g, ' ')}> - a Tecton glyph takes SVG props and ` +
...
const stripped = section('stripped');
...
const log = [
  ...
  `Props stripped:      ${stripped.length}`,
  ...
  '--------------',
  '',
  ...(stripped.length === 0 ? ['  (none)'] : stripped.map(line => `  ${line}`)),
  '',
  'Hand-written rules',
  '------------------',
  ...
  "  The bodies of the ported files are upstream's own prose and sample data.",
  '  Where an example names Astryx in a comment, in UI copy or inside a code',
  '  sample it still says Astryx: the port rewrites imports, not content. Only',
  '  the doc objects beside the examples are rewritten (`@astryxdesign/core` ->',
  '  `@tecton/react`, `Astryx` -> `Tecton`, `astryx` -> `tecton`).',
