...
import {ComplexSelector} from '@tecton/react/ComplexSelector';
import {ProgressBar} from '@tecton/react/ProgressBar';
import {ResizeHandle, useResizable} from '@tecton/react/Resizable';
...
import {PowerSearch, usePowerSearchConfig} from '@tecton/react/PowerSearch';
import type {PowerSearchFilter} from '@tecton/react/PowerSearch';
import {
  Table,
  pixel,
  proportional,
...
} from '@tecton/react/Table';
import type {TableColumn, TablePlugin} from '@tecton/react/Table';
import {
  AddIcon,
  AddPinIcon,
  CancelCircleIcon,
...
const BookmarkFilled = (props: SVGProps<SVGSVGElement>) => (
  <AddPinIcon {...props} fill="currentColor" />
);
const SlidersHorizontalFilled = (props: SVGProps<SVGSVGElement>) => (
  <ControlsIcon {...props} fill="currentColor" />
);
...
const BULK_EDIT_ACTIONS: ReadonlyArray<{
...
}> = [
  {
    key: 'assign',
    label: 'Assign',
    icon: <Icon icon={PersonIcon} size="sm" />,
    onClick: s => alert(`Assign technician: ${s.map(j => j.id).join(', ')}`),
  },
  {
    key: 'reschedule',
    label: 'Reschedule',
    icon: <Icon icon={HistoryIcon} size="sm" />,
...
const VIEW_SECTIONS: ReadonlyArray<{
...
}> = [
  {
    key: 'columns',
    label: 'Columns',
    icon: <Icon icon={ViewColumnIcon} size="sm" />,
    title: 'Columns',
  },
  {
    key: 'density',
  ...
  },
  {
    ...
    icon: <Icon icon={AddPinIcon} size="sm" />,
          ...
          <VStack gap={4}>
            <HStack gap={2}>
              ...
              <IconButton
                ...
                variant="ghost"
                size="sm"
                icon={<Icon icon={CloseIcon} size="sm" />}
...
export function Page() {
  ...
  const powerSearchToggle = (
    <ToggleButton
      ...
      icon={<Icon icon={ControlsIcon} size="sm" />}
            ...
            <HStack gap={0} minHeight={0} xstyle={styles.transferPanels}>
              ...
              >
                <HStack
                  ...
                  hAlign="between"
                  paddingBlock={2}
                  xstyle={[styles.transferPanelHeader, styles.transferPadStart]}
                >
                  <Text
                          ...
                          <Item
                            ...
                            startContent={
                              <IconButton
                                ...
                                size="sm"
                                icon={
                                  <Icon icon={DragIndicatorIcon} size="sm" />
                                }
                                xstyle={[
                                  styles.transferGrip,
  ...
  const viewOptionsTrigger = (
    <Button
      ...
      variant="ghost"
      size="sm"
      endContent={<Icon icon={ChevronDownIcon} size="sm" />}
