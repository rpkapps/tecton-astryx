...
export const docs = {
  ...
  props: [
    ...
    },
    {
      ...
      type: 'DateRange | null',
      description:
        'Selected date range or null. Do NOT redeclare your own DateRange type; use the exported one so TypeScript structurally matches.',
      required: true,
    },
    {
      name: 'onChange',
    ...
    },
    {
      ...
      type: 'ISODateString',
      description:
        'Minimum selectable date. `ISODateString` is a template literal type (`\\`${number}${number}${number}${number}-${number}${number}-${number}${number}\\``). Pass a string literal like `"2026-01-28"`, not a runtime string variable.',
    },
    {
      name: 'max',
      type: 'ISODateString',
